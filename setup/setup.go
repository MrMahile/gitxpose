...
func InitConfig() {
	...
	configDir := filepath.Join(usr.HomeDir, ".config", "gitxpose")
	...
	// Destination paths in the ~/.config/gitxpose
	...
	if _, err := os.Stat(destConfig); err == nil {
		...
		// Copy the config file to ~/.config/gitxpose
	...
	if _, err := os.Stat(destToken); err == nil {
		...
		// Copy the token file to ~/.config/gitxpose
