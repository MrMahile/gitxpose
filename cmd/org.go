...
$ cat orgnames.txt | gitxpose org -c
$ cat orgnames.txt | gitxpose org --info
$ cat orgnames.txt | gitxpose org --delay 1ns
$ cat orgnames.txt | gitxpose org --config custompath/config.yaml -t custompath/github-token.txt`,
...
func init() {
	...
	orgCmd.Flags().StringVar(&orgconfigPath, "config", "$HOME/.config/gitxpose/config.yaml", "path to the config.yaml file")
	orgCmd.Flags().StringVarP(&orgtokenFile, "token", "t", "$HOME/.config/gitxpose/github-token.txt", "Path to the file containing GitHub tokens, 1 token per line")
