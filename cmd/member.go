...
$ cat orgnames.txt | gitxpose member -c
$ cat orgnames.txt | gitxpose member --info
$ cat orgnames.txt | gitxpose member --delay 1ns
$ cat orgnames.txt | gitxpose member --config custompath/config.yaml -t custompath/github-token.txt`,
...
func init() {
	...
	memberCmd.Flags().StringVar(&memberconfigPath, "config", "$HOME/.config/gitxpose/config.yaml", "path to the config.yaml file")
	memberCmd.Flags().StringVarP(&membertokenFile, "token", "t", "$HOME/.config/gitxpose/github-token.txt", "Path to the file containing GitHub tokens, 1 token per line")
