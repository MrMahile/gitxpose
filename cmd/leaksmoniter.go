...
func executeLeaksMonitor() {
	...
	// Load config
	configPath := "$HOME/.config/gitxpose/config.yaml"
	...
	for scanner.Scan() {
		...
		for _, currentScanType := range validScanTypes {
			...
			outputDir := filepath.Join(homeDir, ".gitxpose")
	...
	if downloadDir == "" {
		homeDir, _ := os.UserHomeDir()
		downloadDir = filepath.Join(homeDir, ".gitxpose")
	}
...
func init() {
	...
	// Define flags
	leaksmoniterCmd.Flags().StringVarP(&tokenFile, "token", "t", "$HOME/.config/gitxpose/github-token.txt", "GitHub tokens file, 1 token per line")
