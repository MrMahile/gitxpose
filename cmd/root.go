...
import (
	...
	"github.com/mrmahile/gitxpose/banner"
...
func checkAndUpdateTool() {
	...
	fmt.Printf("Updating gitxpose from version %s to %s...\n", currentVersion, latestVersion)
	cmd := exec.Command("go", "install", "github.com/mrmahile/gitxpose@latest")
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr

	err = cmd.Run()
	if err != nil {
		fmt.Println("Error updating gitxpose:", err)
		os.Exit(1)
	}

	fmt.Println("gitxpose has been updated to the latest version.")
}

// Function to get the current version by executing 'gitxpose -v'
func getCurrentVersion() (string, error) {
	cmd := exec.Command("gitxpose", "-v")
	...
	re := regexp.MustCompile(`Current gitxpose version (v[0-9]+\.[0-9]+\.[0-9]+)`)
...
func getLatestVersion() (string, error) {
	...
	resp, err := http.Get("https://raw.githubusercontent.com/mrmahile/gitxpose/refs/heads/main/banner/banner.go")
	if err != nil {
		return "", err
	}
...
func init() {
	// Define flags
	rootCmd.Flags().BoolP("update", "u", false, "update gitxpose to latest version")
