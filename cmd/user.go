...
$ cat usernames.txt | gitxpose user -c
$ cat usernames.txt | gitxpose user --info
$ cat usernames.txt | gitxpose user --delay 1ns
$ cat usernames.txt | gitxpose user --config custompath/config.yaml -t custompath/github-token.txt`,
...
func init() {
	...
	userCmd.Flags().StringVar(&userconfigPath, "config", "$HOME/.config/gitxpose/config.yaml", "path to the config.yaml file")
	userCmd.Flags().StringVarP(&usertokenFile, "token", "t", "$HOME/.config/gitxpose/github-token.txt", "Path to the file containing GitHub tokens, 1 token per line")
